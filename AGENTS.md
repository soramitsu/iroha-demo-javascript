...
- **Kaigi (`/kaigi`)**: Browser-native 1:1 audio/video call screen for wallet users. Kaigi now starts with a meeting-link flow: hosts create a scheduled meeting, generate a unique `iroha://kaigi/join?...` / `#/kaigi?invite=...` invite, and share that link instead of copy/pasting the initial offer. Live wallets submit `CreateKaigi` / `JoinKaigi` / `EndKaigi` transactions through the preload bridge and carry encrypted answer metadata on-chain so the host can auto-apply the first answer by polling account transactions. Local-only wallets still use the meeting-link UI, but they fall back to the Advanced raw packet section for manual answer exchange. Browser media transport remains self-contained inside this Electron app for now.
