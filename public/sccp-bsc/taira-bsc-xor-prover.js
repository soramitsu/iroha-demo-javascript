...
const loadMaterial = async (row, state, direction) => {
  ...
  const material = {
    ...
    nativeEvmProverBundleHash,
    proofArtifactBytes: proofArtifact.bytes.slice(),
    proofArtifactHash: proofArtifact.sha256,
    provingKeyBytes: provingKey.bytes.slice(),
...
const buildContext = ({ direction, request, material, config, state }) =>
  Object.freeze({
    ...
    proofArtifactBytes: material.proofArtifactBytes.slice(),
    provingKeyBytes: material.provingKeyBytes.slice(),
